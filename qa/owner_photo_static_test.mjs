...
const client=fs.readFileSync('collection-owner-photo.js','utf8');
...
assert.match(client,/if\(error\)\{await cleanup\(path\);throw error\}/);
assert.match(client,/MAX_BYTES=8\*1024\*1024/);

assert.match(client,/data-exchangeable/);
assert.match(client,/ensureLegacyPhoto/);
...
assert.match(client,/hydrateCounterpartyPhotos/);
...
const appIndex=html.indexOf('/app-v3.js');
const ownerIndex=html.indexOf('/collection-owner-photo.js');
assert.ok(appIndex>=0 && ownerIndex>appIndex,'collection-owner-photo.js must load after app-v3.js');

...
const S={
  user:null,profile:null,founder:null,liquidity:null,membership:null,collection:[],wishlist:[],matches:[],
...
function clearProtectedState(){S.user=null;S.profile=null;S.collection=[];S.wishlist=[];S.matches=[];S.requests=[];S.exchanges=[];S.notifications=[];S.messages=[];S.reviews=[];S.founder=null;S.liquidity=null;S.membership=null;S.profiles={};S.items={};S.sets={}}
...
async function refreshCore(){
  ...
  const uid=S.user.id;
  const [p,c,w,req,ex,n,msg,found,liq,membership]=await Promise.all([
  ...
  const failures=[p,c,w,req,ex,n,msg,found,liq,membership].filter(result=>result.error);
...
function membershipPolicy(){
  const status=S.membership;if(!status)return null;
  const founding=Number(status.founding_remaining||0),early=Number(status.early_remaining||0);
  if(founding>0)return {body:'The first 100 members receive complimentary marketplace membership for life. Members #101–#1000 become Early Members and use BrickCircle free throughout beta.'};
  if(early>0)return {body:'The Founding 100 is complete. Members #101–#1000 are Early Members with complimentary access throughout beta while local BrickCircles build liquidity.'};
  return {body:'Membership remains free during beta. Paid membership will be introduced city-by-city only after local marketplace liquidity is strong enough to deliver real value.'};
}
function membershipBadge(){const status=S.membership;if(!status?.my_tier)return '';if(status.my_tier==='founding')return `★ Founding Member #${status.my_number}`;if(status.my_tier==='early')return `Early Member #${status.my_number} · Free during beta`;return status.beta_free?'Beta Member · Free during beta':'BrickCircle Member'}
function membershipCityCopy(){
  ...
  if(!status.city_pricing_eligible)return `<strong>Pricing is not ready in ${esc(status.my_city)}.</strong> ${esc(gate)}.`;
  if(!status.city_pricing_enabled)return `<strong>${esc(status.my_city)} has reached the liquidity threshold, but paid launch is still disabled.</strong> Pricing requires an explicit launch decision.`;
...
function firstMatchCoach(){
  ...
  const guidance={own:'Add 3 LEGO sets you own so BrickCircle has enough supply signals to work with.',want:'Add 3 sets you genuinely want to experience.',exchangeable:'Make 1 owned set available so it can participate in reciprocal matching.',match:'Your collection is ready. Explore more sets while BrickCircle looks for reciprocal overlap.'};
...
window.bcNav=navigate;window.bcAuth=showAuth;window.bcClose=closeOverlay;window.bcV3Refresh=refreshRoute;window.bcPushToast=toast;

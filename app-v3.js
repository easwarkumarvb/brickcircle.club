...
async function refreshCore(){
  ...
  const uid=S.user.id;
  const [p,c,w,ex,n,msg,liq,membership]=await Promise.all([
  ...
  const failures=[p,c,w,ex,n,msg,liq,membership].filter(result=>result.error);
  if(!p.error)S.profile=p.data||null;if(!c.error)S.collection=c.data||[];if(!w.error)S.wishlist=w.data||[];if(!ex.error){S.exchanges=ex.data||[];S.requests=S.exchanges.filter(row=>row.state==='PROPOSED')}if(!n.error)S.notifications=n.data||[];if(!msg.error)S.messages=msg.data||[];
...
async function releaseSetFromWorkflow(itemId){
  ...
  if(['ACTIVE','HANDOFF_PENDING','HANDOFF_ISSUE','EARLY_RETURN','RETURN_PLANNING','RETURN_INSPECTION','DISPUTED'].includes(workflow.state)){toast('Physical custody may have changed. Continue in the exchange case so both sets remain protected.');return navigate('exchange',workflow.id)}
  const action=workflow.state==='PROPOSED'?(workflow.proposer_id===S.user.id?'withdraw':'decline'):'cancel';
  if(!confirm(`End this pre-handoff exchange for ${item.lego_sets?.name||item.set_number}? The case history and conversation remain preserved.`))return;
  const {error}=await db.rpc('exchange_case_transition',{p_case_id:workflow.id,p_expected_version:workflow.state_version,p_action:action,p_idempotency_key:crypto.randomUUID(),p_payload:{reason:'Owner ended the exchange from My Sets'}});
  if(error)return fail(error,'Could not safely end this exchange. Open the case and try again.');
  await refreshCore();renderSetsBody();toast('Exchange ended safely. Set preferences were restored.');
}
...
function showProposal(i){const m=S.matches[i];if(!m)return;const p=S.profiles[m.match_user]||{};const o=modal(`<div class="bc-modal-head"><div><span class="bc-pill green">Reciprocal match</span><h2 style="margin-top:8px">Propose an exchange with ${esc(p.display_name||'this collector')}</h2><p class="bc-muted">Local, in-person and inspection-first.</p></div><button class="bc-close" data-close>×</button></div><div class="bc-notice"><b>${esc(m.offered_name)}</b> ⇄ <b>${esc(m.requested_name)}</b></div><form class="bc-form" id="bc-proposal"><div class="bc-field"><label>Temporary exchange period</label><select class="bc-select" name="days"><option value="30">30 days</option><option value="60" selected>60 days</option><option value="90">90 days</option></select></div><div class="bc-field"><label>Message</label><textarea class="bc-textarea" name="message">Hi! We have a reciprocal BrickCircle match. Would you like to meet locally, inspect both sets and exchange them temporarily?</textarea></div><div class="bc-notice warn"><b>What happens next</b><br>You both use one shared case for proposal, meetup, inspection, handoff, return and messages.</div><div class="bc-form-actions"><button type="button" class="bc-btn" data-close>Cancel</button><button type="submit" class="bc-btn primary">Send proposal</button></div></form>`);$$('[data-close]',o).forEach(b=>b.onclick=closeOverlay);$('#bc-proposal',o).onsubmit=async e=>{e.preventDefault();const f=new FormData(e.currentTarget),btn=$('button[type="submit"]',e.currentTarget);if(btn.disabled)return;btn.disabled=true;btn.textContent='Sending…';const {data,error}=await db.rpc('create_exchange_case',{p_offered_item_id:m.offered_item,p_requested_item_id:m.requested_item,p_duration_days:Number(f.get('days')),p_message:String(f.get('message')||''),p_idempotency_key:crypto.randomUUID()});if(error||data?.ok===false){fail(error||new Error('Could not send the proposal.'));btn.disabled=false;btn.textContent='Send proposal';return}closeOverlay();await refreshCore();navigate('exchange',data.case?.id);toast(data?.idempotent?'Your existing proposal is open.':'Exchange proposal sent.');track('exchange_proposal_sent',{match_user:m.match_user,duration_days:Number(f.get('days'))})}}
...
async function respondRequest(id,action){
  ...
  const {data,error}=await settledTimeout(db.rpc('exchange_case_transition',{p_case_id:id,p_expected_version:exchange.state_version,p_action:action,p_idempotency_key:crypto.randomUUID(),p_payload:{}}),15000);
  if(error){
    fail(error);
...
function renderExchangeDetailLoaded(e,messages,events){const p=otherProfile(e),mineA=e.user_a===S.user.id,myItem=mineA?e.item_a:e.item_b,theirItem=mineA?e.item_b:e.item_a,myName=itemName(myItem),theirName=itemName(theirItem),messagingClosed=terminalCaseStates.has(e.state),next=caseNextAction(e),secondary=caseSecondaryAction(e);page(`<button class="bc-back" data-action="exchanges">← Back to Exchanges</button><section class="bc-exchange-hero"><div class="bc-exchange-hero-top"><div><span class="bc-pill gold">${esc(exchangeStageLabel(e))} · with ${esc(p.display_name||'Collector')}${p.city?' · '+esc(p.city):''}</span><h1>${esc(myName)} ⇄ ${esc(theirName)}</h1><p>${e.duration_days}-day local temporary exchange${e.return_due_at?` · return due ${fmtDate(e.return_due_at)}`:''}</p></div></div>${exchangeTimeline(e)}</section><div class="bc-exchange-layout"><div><section class="bc-card bc-flow-card" id="bc-flow">${flowMarkup(e,events)}</section></div><aside><section class="bc-card bc-flow-card"><h2>Exchange conversation</h2><div class="bc-chat" id="bc-chat">${messages.length?messages.map(m=>`<div class="bc-msg ${m.sender_id===S.user.id?'mine':''}">${esc(m.body)}<small>${fmtDateTime(m.created_at)}</small></div>`).join(''):'<div class="bc-small">No messages yet. Keep meetup, handoff, return and issue details here.</div>'}</div>${messagingClosed?'':`<form class="bc-chat-form" id="bc-chat-form"><input name="message" maxlength="4000" placeholder="Message ${attr((p.display_name||'collector').split(' ')[0])}" required><button class="bc-btn primary">Send</button></form>`}</section><section class="bc-card bc-flow-card" style="margin-top:14px"><h2>What happens next</h2><p>${esc(next.copy)}</p>${next.action?`<button class="bc-btn primary" data-case-action="${attr(next.action)}">${esc(next.label)}</button>`:''}${secondary?`<button class="bc-btn" style="margin-left:7px" data-case-action="${attr(secondary.action)}">${esc(secondary.label)}</button>`:''}${!messagingClosed?'<button class="bc-btn danger" style="margin-top:10px" data-case-action="report_issue">Report an issue</button>':''}<div class="bc-notice warn" style="margin-top:12px"><b>Inspection first.</b><br>Meet publicly. Inspect condition, parts and completeness together. Confirm handoff only after you are satisfied.</div></section></aside></div>`);bindCommon(app());bindExchangeDetail(e,p)}
...
function caseNextAction(e){const mine=S.user.id,recipient=e.recipient_id===mine,meetupOther=e.meetup_proposed_by&&e.meetup_proposed_by!==mine,returnOther=e.return_proposed_by&&e.return_proposed_by!==mine;
  ...
  if(['HANDOFF_ISSUE','DISPUTED'].includes(e.state))return {action:null,label:'',copy:'The case is paused. Keep communicating here while custody or the reported issue is resolved.'};
...
function caseSecondaryAction(e){if(e.state==='PROPOSED')return {action:e.proposer_id===S.user.id?'withdraw':'decline',label:e.proposer_id===S.user.id?'Withdraw':'Decline'};if(['ACCEPTED','MEETUP_PLANNING','MEETUP_CONFIRMED','INSPECTION','HANDOFF_PENDING'].includes(e.state))return {action:'cancel',label:'Cancel before handoff'};if(e.state==='ACTIVE')return {action:'early_return',label:'Request early return'};return null}
...
function bindExchangeDetail(e,p){$$('[data-case-action]',app()).forEach(button=>button.onclick=()=>runCaseAction(e,button.dataset.caseAction,button));const form=$('#bc-chat-form');if(form)form.onsubmit=async event=>{event.preventDefault();const data=new FormData(form),body=String(data.get('message')||'').trim(),button=$('button',form);if(!body||button.disabled)return;button.disabled=true;const {error}=await db.rpc('send_exchange_case_message',{p_case_id:e.id,p_body:body,p_idempotency_key:crypto.randomUUID()});if(error){button.disabled=false;return fail(error,'Message was not sent. Please retry.')}form.reset();await refreshCore();await renderExchangeDetail(e.id,S.renderToken)}}
function scheduleCaseMeetup(e,returning=false){const title=returning?'Plan return meetup':'Plan public meetup',o=modal(`<div class="bc-modal-head"><div><h2>${title}</h2><p class="bc-muted">The other collector must accept the same public place and future time.</p></div><button class="bc-close" data-close>×</button></div><form class="bc-form" id="bc-case-meetup"><div class="bc-field"><label>Public venue</label><input class="bc-input" name="venue" value="${attr(returning?e.return_venue_name||'':e.meetup_venue_name||'')}" required></div><div class="bc-field"><label>Area / neighbourhood</label><input class="bc-input" name="area" value="${attr(returning?e.return_venue_area||'':e.meetup_venue_area||'')}"></div><div class="bc-field"><label>Date & time</label><input class="bc-input" name="when" type="datetime-local" required></div><button class="bc-btn primary">Share proposal</button></form>`);$$('[data-close]',o).forEach(button=>button.onclick=closeOverlay);$('#bc-case-meetup',o).onsubmit=async event=>{event.preventDefault();const data=new FormData(event.currentTarget),date=new Date(String(data.get('when'))),button=$('button[type="submit"]',event.currentTarget);if(Number.isNaN(date.getTime())||date<=new Date())return toast('Choose a future date and time.');button.disabled=true;const {error}=await db.rpc('exchange_case_transition',{p_case_id:e.id,p_expected_version:e.state_version,p_action:returning?'propose_return':'propose_meetup',p_idempotency_key:crypto.randomUUID(),p_payload:{venue_name:String(data.get('venue')||'').trim(),venue_area:String(data.get('area')||'').trim(),meetup_at:date.toISOString()}});if(error){button.disabled=false;return fail(error,'Could not save the meetup proposal.')}closeOverlay();await refreshCore();await renderExchangeDetail(e.id,S.renderToken)}}
async function runCaseAction(e,action,button){if(action==='propose_meetup')return scheduleCaseMeetup(e,false);if(action==='propose_return')return scheduleCaseMeetup(e,true);if(action==='report_issue'){const note=prompt('Describe the loss, damage, missing parts, no-show, safety concern, or disagreement.');if(!note)return;return performCaseTransition(e,'report_issue',{issue_type:'other',note},button)}if(action==='handoff'&&!confirm('Confirm only after the physical set has changed custody and you are satisfied with inspection. Continue?'))return;if(action==='return_confirm'&&!confirm('Confirm only after your own physical LEGO set is back and inspected. Continue?'))return;if(action==='cancel'&&!confirm('Cancel before handoff? Set preferences will be restored and the case history preserved.'))return;return performCaseTransition(e,action,{},button)}
async function performCaseTransition(e,action,payload,button){if(button)button.disabled=true;const {error}=await db.rpc('exchange_case_transition',{p_case_id:e.id,p_expected_version:e.state_version,p_action:action,p_idempotency_key:crypto.randomUUID(),p_payload:payload});if(error){if(button)button.disabled=false;return fail(error,'The exchange changed or this action is no longer available. Refresh and retry.')}await refreshCore();await renderExchangeDetail(e.id,S.renderToken);toast('Exchange case updated.')}

...
function cleanQuery(){if(location.search){history.replaceState({},'',location.pathname+(location.hash||'#home'))}}
...
function syncNav(){
  $$('[data-nav]').forEach(b=>b.classList.toggle('active',b.dataset.nav===routeName()));
}
...
async function oauth(provider){
  const {error}=await db.auth.signInWithOAuth({provider,options:{redirectTo:`${location.origin}/v2.html?oauth=1`}});if(error)fail(error,`${provider} sign-in is unavailable.`)
}
function showAuth(){
  ...
  $('[data-close]',o).onclick=closeOverlay;$$('[data-oauth]',o).forEach(b=>b.onclick=()=>oauth(b.dataset.oauth));$$('[data-auth-tab]',o).forEach(b=>b.onclick=()=>{const mode=b.dataset.authTab;$$('[data-auth-tab]',o).forEach(x=>x.classList.toggle('active',x===b));renderEmailAuth(mode,o)});renderEmailAuth('signin',o);track('auth_opened');
}
function renderEmailAuth(mode,o=document){
  const host=$('#bc-auth-email',o);if(!host)return;
  host.innerHTML=mode==='signin'?`<form class="bc-form" id="bc-email-signin"><div class="bc-field"><label>Email</label><input class="bc-input" name="email" type="email" autocomplete="email" required></div><div class="bc-field"><label>Password</label><input class="bc-input" name="password" type="password" autocomplete="current-password" minlength="6" required></div><button class="bc-btn primary">Sign in</button><button class="bc-btn ghost" type="button" data-forgot>Forgot password?</button></form>`:`<form class="bc-form" id="bc-email-signup"><div class="bc-field"><label>Collector name</label><input class="bc-input" name="name" autocomplete="name" required></div><div class="bc-field"><label>Email</label><input class="bc-input" name="email" type="email" autocomplete="email" required></div><div class="bc-field"><label>Password</label><input class="bc-input" name="password" type="password" autocomplete="new-password" minlength="6" required></div><button class="bc-btn primary">Create account</button></form>`;
  $('#bc-email-signin',host)?.addEventListener('submit',async e=>{e.preventDefault();const f=new FormData(e.currentTarget),btn=$('button[type="submit"]',e.currentTarget);btn.disabled=true;btn.textContent='Signing in…';const {error}=await db.auth.signInWithPassword({email:String(f.get('email')),password:String(f.get('password'))});if(error){fail(error);btn.disabled=false;btn.textContent='Sign in'}else closeOverlay()});
  ...
  $('#bc-email-signup',host)?.addEventListener('submit',async e=>{e.preventDefault();const f=new FormData(e.currentTarget),btn=$('button[type="submit"]',e.currentTarget);btn.disabled=true;btn.textContent='Creating account…';const {data,error}=await db.auth.signUp({email:String(f.get('email')),password:String(f.get('password')),options:{data:{full_name:String(f.get('name'))},emailRedirectTo:`${location.origin}/v2.html`}});if(error){fail(error);btn.disabled=false;btn.textContent='Create account';return}closeOverlay();toast(data.session?'Account created.':'Account created — check your email to confirm.');});
}
...
function showOnboarding(){
  ...
  const form=$('#bc-onboard',o),country=$('[name="country"]',form),city=$('[name="city"]',form);country.onchange=()=>{city.innerHTML=cityOptions(country.value);city.disabled=!country.value};form.onsubmit=async e=>{e.preventDefault();const f=new FormData(form),btn=$('button',form);btn.disabled=true;btn.textContent='Saving…';const patch={display_name:String(f.get('name')).trim(),country:String(f.get('country')).trim(),city:String(f.get('city')).trim(),updated_at:new Date().toISOString()};const {error}=await db.from('profiles').update(patch).eq('id',S.user.id);if(error){fail(error);btn.disabled=false;btn.textContent='Continue to add my LEGO sets';return}S.profile={...(S.profile||{}),...patch};closeOverlay();await refreshCore();S.setTab='collection';navigate('browse');toast('Great — now add at least 3 LEGO sets you own.');track('onboarding_location_completed',patch)};
}
...
function readiness(){
  const l=S.liquidity||{},steps=[
    ...
    {done:Number(l.exchangeable_count||S.collection.filter(x=>x.available_for_exchange).length)>=2,title:'2 exchangeable',desc:`${Number(l.exchangeable_count||S.collection.filter(x=>x.available_for_exchange).length)}/2`,go:'sets'},
...
async function renderHome(token){
  if(!S.user){
    page(`<section class="bc-hero"><div><span class="bc-pill gold">Founding 100 · 18+ AFOL community</span><h1>Experience more LEGO <em>without owning every set.</em></h1><p>Catalogue the sets you own, wishlist what you want to experience, find a reciprocal collector in your city, meet safely and exchange temporarily.</p><div class="bc-hero-actions"><button class="bc-btn primary" data-auth>Join the Founding 100</button><button class="bc-btn" data-action="browse">Explore collectible sets</button></div><div class="bc-hero-proof"><div><b>🧱 Built for adult collectors</b><span class="bc-small">High-end sets, condition and reputation</span></div><div><b>⇄ Reciprocal matching</b><span class="bc-small">Both collectors want what the other owns</span></div><div><b>🤝 Meet before handoff</b><span class="bc-small">Inspect both sets in person</span></div><div><b>↩ Temporary exchange</b><span class="bc-small">Return, inspect and review</span></div></div></div><div class="bc-concept"><h3>ONE SIMPLE LOOP</h3><div class="bc-concept-row"><div class="bc-concept-set"><div class="emoji">🏎️</div><strong>Your Ferrari</strong><small>Available</small></div><div class="bc-swapmark">⇄</div><div class="bc-concept-set"><div class="emoji">🚀</div><strong>Their Saturn V</strong><small>On your wishlist</small></div></div><div class="bc-concept-foot"><b>BrickCircle finds the overlap.</b> You both meet, inspect, swap for the agreed period, then return and review.</div></div></section><div class="bc-howstrip"><div><b>1 · List</b><span>Add what you own</span></div><div><b>2 · Wishlist</b><span>Choose what you want</span></div><div><b>3 · Match</b><span>Find a local reciprocal fit</span></div><div><b>4 · Exchange</b><span>Meet, swap, return</span></div></div>`);bindCommon(app());$$('[data-auth]',app()).forEach(b=>b.onclick=showAuth);return;
  }
  const rd=readiness(),active=activeExchanges(),pending=pendingRequests(),matches=S.matches||[],found=S.founder||{},l=S.liquidity||{};
  const name=(S.profile?.display_name||S.user.email||'Collector').split(' ')[0];
  const next=rd.next;page(`<div class="bc-dashboard-hero"><section class="bc-welcome"><span class="bc-pill gold">${found.my_is_founder&&found.my_number?`★ Founding Member #${found.my_number}`:'BrickCircle member'}</span><h1>Welcome back, ${esc(name)}.</h1><p>${rd.score>=100?'You’re Match Ready. Keep your collection current and check for reciprocal matches.':'Complete your Match Ready setup so BrickCircle can find the strongest local exchanges for you.'}</p><div class="bc-head-actions" style="justify-content:flex-start;margin-top:15px"><button class="bc-btn gold" data-action="${matches.length?'matches':next?.go||'browse'}">${matches.length?`See ${matches.length} match${matches.length===1?'':'es'}`:next?.title||'Explore sets'}</button>${S.collection.length?'<button class="bc-btn ghost" style="color:#fff;border-color:#ffffff44" data-action="sets">My sets</button>':''}</div></section><section class="bc-card bc-readiness"><div class="bc-small">MATCH READINESS</div><div class="bc-readiness-score">${clamp(rd.score,0,100)}%</div><div class="bc-progress"><span style="width:${clamp(rd.score,0,100)}%"></span></div><div class="bc-small">${rd.done}/5 activation steps complete</div></section></div><div class="bc-checklist">${rd.steps.map(x=>`<div class="bc-check ${x.done?'done':''}"><b>${x.done?'✓ ':''}${esc(x.title)}</b><span>${esc(x.desc)}</span>${!x.done?`<button class="bc-btn ghost" style="padding:5px 0;min-height:32px;color:#175cd3;border:0" data-ready="${x.invite?'invite':x.go}">Do this →</button>`:''}</div>`).join('')}</div>${next?`<div class="bc-next"><div><strong>Best next step: ${esc(next.title)}</strong><small>${esc(next.desc)}</small></div><button class="bc-btn primary" data-ready="${next.invite?'invite':next.go}">${next.invite?'Invite an AFOL':'Continue'}</button></div>`:''}<div class="bc-dashboard-grid"><section class="bc-card bc-dash-card"><h3>Reciprocal matches</h3><div class="bc-statbig">${matches.length}</div><p class="bc-small">Collectors in your city who want what you can offer and own something on your wishlist.</p><button class="bc-btn" data-action="matches">Open matches</button></section><section class="bc-card bc-dash-card"><h3>Exchange activity</h3><div class="bc-statbig">${active.length}</div><p class="bc-small">${pending.length?`${pending.length} pending request${pending.length===1?'':'s'} also waiting.`:'No pending requests right now.'}</p><button class="bc-btn" data-action="exchanges">Open exchanges</button></section><section class="bc-card bc-dash-card"><h3>${esc(S.profile?.city||'Your city')} BrickCircle</h3><div class="bc-statbig">${Number(l.city_members||0)}</div><p class="bc-small">${Number(l.city_exchangeable_sets||0)} exchangeable sets · ${Number(l.city_wishlist_items||0)} wishlist signals</p><button class="bc-btn" data-invite>Invite local AFOL</button></section></div>`);
...
async function renderSets(token,legacy='sets'){
  ...
  const rd=readiness();page(`<div class="bc-page-head"><div><h1>My Sets</h1><p>Keep the relationship simple: what you own, and what you want to experience next.</p></div><div class="bc-head-actions">${pill(`${rd.score}% Match Ready`,rd.score>=100?'green':'gold')}<button class="bc-btn primary" data-action="browse">+ Add sets</button></div></div><div class="bc-tabs"><button class="bc-tab ${S.setTab==='collection'?'active':''}" data-settab="collection">My Collection · ${S.collection.length}</button><button class="bc-tab ${S.setTab==='wishlist'?'active':''}" data-settab="wishlist">Wishlist · ${S.wishlist.length}</button></div><div id="bc-sets-body"></div>`);$$('[data-settab]').forEach(b=>b.onclick=()=>{S.setTab=b.dataset.settab;renderSets(S.renderToken)});renderSetsBody();
}
function renderSetsBody(){
  const host=$('#bc-sets-body');if(!host)return;if(S.setTab==='wishlist'){host.innerHTML=S.wishlist.length?`<div class="bc-set-list">${S.wishlist.map(w=>mySetRow(w,true)).join('')}</div>`:empty('♡','Build your wishlist','Add at least three sets you would genuinely like to experience. This gives BrickCircle enough demand signals to find reciprocal collectors.','Browse LEGO sets','browse')}else{host.innerHTML=S.collection.length?`<div class="bc-notice warn" style="margin-bottom:12px"><b>Make at least 2 sets exchangeable.</b> Only sets you explicitly mark available can appear in reciprocal matches.</div><div class="bc-set-list">${S.collection.map(c=>mySetRow(c,false)).join('')}</div>`:empty('🧱','Start with the LEGO you already own','Add at least three collectible sets. You decide which physical copies are available for a temporary local exchange.','Add my first set','browse')};bindCommon(host);$$('[data-exchangeable]',host).forEach(x=>x.onchange=()=>setExchangeable(x.dataset.exchangeable,x.checked));$$('[data-remove-wish]',host).forEach(b=>b.onclick=()=>removeWish(b.dataset.removeWish));$$('[data-edit-set]',host).forEach(b=>b.onclick=()=>editCollectionItem(b.dataset.editSet));wireImages(host)
}
function mySetRow(row,wish){const s=row.lego_sets||S.sets[row.set_number]||{};return `<article class="bc-card bc-myset"> <div class="bc-myset-visual"><img src="https://images.brickset.com/sets/images/${attr(row.set_number)}-1.jpg" alt="" loading="lazy" data-set-image="${attr(row.set_number)}"><div hidden>🧱</div></div><div><h3>${esc(s.name||row.set_number)}</h3><div class="bc-myset-meta">Set ${esc(row.set_number)}${s.theme?` · ${esc(s.theme)}`:''}${s.piece_count?` · ${Number(s.piece_count).toLocaleString()} pieces`:''}</div><div class="bc-statusrow">${wish?pill('Wishlist','blue'):(row.available_for_exchange?pill('Exchangeable','green'):pill('Not exchangeable'))}${!wish&&row.condition?pill(row.condition):''}${!wish&&row.completeness!=null?pill(`${row.completeness}% complete`):''}</div></div><div class="bc-myset-actions">${wish?`<button class="bc-btn" data-remove-wish="${attr(row.set_number)}" aria-label="Remove ${attr(s.name||row.set_number)} from wishlist">Remove</button>`:`<label class="bc-toggle"><input type="checkbox" data-exchangeable="${row.id}" ${row.available_for_exchange?'checked':''}> Exchangeable</label><button class="bc-btn" data-edit-set="${row.id}">Details</button>`}</div></article>`}
...
function editCollectionItem(id){const c=S.collection.find(x=>x.id===id);if(!c)return;const o=modal(`<div class="bc-modal-head"><div><h2>${esc(c.lego_sets?.name||c.set_number)}</h2><p class="bc-muted">Describe the physical copy another collector would receive.</p></div><button class="bc-close" data-close>×</button></div><form class="bc-form" id="bc-edit-item"><div class="bc-field"><label>Condition</label><select class="bc-select" name="condition">${['Excellent','Very good','Good','Fair'].map(x=>`<option ${c.condition===x?'selected':''}>${x}</option>`).join('')}</select></div><div class="bc-field"><label>Completeness (%)</label><input class="bc-input" name="completeness" type="number" min="0" max="100" value="${Number(c.completeness??100)}"></div><label><input name="box" type="checkbox" ${c.original_box?'checked':''}> Original box included</label><label><input name="exchangeable" type="checkbox" ${c.available_for_exchange?'checked':''}> Available for exchange</label><div class="bc-field"><label>Notes</label><textarea class="bc-textarea" name="notes" placeholder="Missing pieces, sticker condition, instructions, etc.">${esc(c.notes||'')}</textarea></div><div class="bc-form-actions"><button type="button" class="bc-btn" data-close>Cancel</button><button class="bc-btn primary">Save details</button></div></form>`);$$('[data-close]',o).forEach(b=>b.onclick=closeOverlay);$('#bc-edit-item',o).onsubmit=async e=>{e.preventDefault();const f=new FormData(e.currentTarget),btn=$('button[type="submit"]',e.currentTarget);btn.disabled=true;const patch={condition:String(f.get('condition')),completeness:Number(f.get('completeness')),original_box:f.get('box')==='on',available_for_exchange:f.get('exchangeable')==='on',notes:String(f.get('notes')||''),updated_at:new Date().toISOString()};const {error}=await db.from('collection_items').update(patch).eq('id',id).eq('user_id',S.user.id);if(error){fail(error);btn.disabled=false;return}closeOverlay();await refreshCore();renderSetsBody();toast('Set details updated.')}
}
...
async function signOut(){try{await db.auth.signOut({scope:'local'})}catch(_){try{await db.auth.signOut()}catch(__){}}try{Object.keys(localStorage).filter(k=>/^bc_(?!pending_referral)/.test(k)).forEach(k=>localStorage.removeItem(k))}catch(_){ }S.user=null;closeOverlay();navigate('home');await refreshRoute();toast('Signed out.')}
...
function showNotifications(){if(!S.user)return showAuth();const unread=S.notifications.filter(n=>!n.read_at).length,o=drawer(`<div class="bc-drawer-head"><div><h2>Notifications</h2><div class="bc-small">${unread?`${unread} unread`:'You’re caught up'}</div></div><button class="bc-close" data-close>×</button></div>${unread?'<button class="bc-btn" data-read-all style="margin-bottom:10px">Mark all read</button>':''}<div>${S.notifications.length?S.notifications.map(n=>`<div class="bc-notification ${n.read_at?'':'unread'}" data-note="${n.id}"><b>${esc(n.title||String(n.kind||'Update').replace(/_/g,' '))}</b><div>${esc(n.body||'')}</div><small>${fmtDateTime(n.created_at)}</small></div>`).join(''):'<div class="bc-empty"><div class="bc-empty-icon">🔔</div><h3>No notifications yet</h3></div>'}</div>`);$('[data-close]',o).onclick=()=>o.closest('.bc-drawer-overlay').remove();$('[data-read-all]',o)?.addEventListener('click',async()=>{const {error}=await db.from('notifications').update({read_at:new Date().toISOString()}).eq('user_id',S.user.id).is('read_at',null);if(error)return fail(error);await refreshCore();shell();await renderRoute();showNotifications()});$$('[data-note]',o).forEach(el=>el.onclick=async()=>{const id=el.dataset.note,n=S.notifications.find(x=>x.id===id);if(n&&!n.read_at)await db.from('notifications').update({read_at:new Date().toISOString()}).eq('id',id).eq('user_id',S.user.id);o.closest('.bc-drawer-overlay').remove();await refreshCore();shell();await renderRoute()})}
...
async function boot(){
  captureReferral();setupPWA();await providerSettings();const {data:{session}}=await db.auth.getSession();S.user=session?.user||null;await refreshCore();S.booted=true;shell();await renderRoute();if(S.user){await claimReferralAndProvider();await refreshCore();shell();await renderRoute();await onboardingIfNeeded()}else if(parseJoinIntent())setTimeout(showAuth,120);if(new URLSearchParams(location.search).has('oauth'))cleanQuery();
  db.auth.onAuthStateChange(async(event,session)=>{const prev=S.user?.id;S.user=session?.user||null;if(event==='SIGNED_IN'&&S.user){await claimReferralAndProvider();await refreshCore();shell();await renderRoute();await onboardingIfNeeded();if(!prev)toast('Welcome to BrickCircle.')}else if(event==='SIGNED_OUT'){await refreshCore();shell();await renderRoute()}});
}

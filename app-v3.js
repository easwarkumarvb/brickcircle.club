...
function stopExchangeRealtime(){clearTimeout(exchangeRefreshTimer);if(!exchangeChannel)return;try{db.removeChannel?.(exchangeChannel)}catch(_){try{exchangeChannel.unsubscribe?.()}catch(__){}}exchangeChannel=null}
function refreshOpenExchange(){clearTimeout(exchangeRefreshTimer);exchangeRefreshTimer=setTimeout(async()=>{if(!S.user)return;await refreshCore();if(routeName()==='exchange'&&routeId())renderExchangeDetail(routeId(),S.renderToken)},180)}
function startExchangeRealtime(){stopExchangeRealtime();if(!S.user?.id||typeof db.channel!=='function')return;exchangeChannel=db.channel(`exchange-workspace:${S.user.id}`).on('postgres_changes',{event:'*',schema:'public',table:'exchanges'},refreshOpenExchange).on('postgres_changes',{event:'*',schema:'public',table:'exchange_meetups'},refreshOpenExchange).on('postgres_changes',{event:'*',schema:'public',table:'exchange_returns'},refreshOpenExchange).on('postgres_changes',{event:'*',schema:'public',table:'messages'},p=>{if((p.new||p.old)?.exchange_id)refreshOpenExchange()}).subscribe()}
...
function exchangePhase(e,meet,ret){if(e.state==='completed')return ['complete','Exchange complete','Both collectors confirmed the return.'];if(e.state==='disputed')return ['paused','Exchange paused','An issue was reported. Messages remain open while actions are paused.'];if(e.state==='accepted'&&!meet)return ['plan','Plan the first meetup','Either collector can choose a public venue and time.'];if(e.state==='accepted')return ['handoff','Meet, inspect and start the swap','Use the same live checklist together.'];if(e.state==='swap_active'&&!ret)return ['return-plan','Plan the return meetup','The temporary swap is active. Arrange the return here.'];return ['return','Return, inspect and complete','Use the same live checklist to confirm both sets return safely.']}
function flowStep(e,obj,action,label,open,why,number){const key=action==='confirm'?'confirmed':action,mine=participantValue(obj,e,key),theirs=otherValue(obj,e,key);return '<div class="bc-flow-step '+(mine?'done':(!open?'locked':''))+'"><div class="bc-flow-step-number">'+(mine?'✓':number)+'</div><div class="bc-flow-step-body"><b>'+esc(label)+'</b><div class="bc-flow-step-status"><span class="'+(mine?'ready':'')+'">You: '+(mine?'confirmed':'not yet')+'</span><span class="'+(theirs?'ready':'')+'">Other collector: '+(theirs?'confirmed':'waiting')+'</span></div>'+(!mine&&open?'<button class="bc-btn primary" style="margin-top:9px" data-flow-action="'+action+'">Confirm my step</button>':'')+(!mine&&!open?'<small>🔒 '+esc(why)+'</small>':'')+'</div></div>'}
function sharedChecklist(e,obj,returning){const safe=obj.safety_ack_a&&obj.safety_ack_b,arrived=obj.arrived_a&&obj.arrived_b,inspected=obj.inspected_a&&obj.inspected_b,rows=returning?[['safety_ack','Confirm safe public return',true,'Both collectors need to confirm the public return plan.'],['arrived','Confirm arrival',safe,'Waiting for both safety confirmations.'],['inspected','Inspect your returned set',arrived,'Waiting for both arrival confirmations.'],['confirm','Confirm your set is back',inspected,'Waiting for both inspections.']]:[['safety_ack','Confirm safe public meetup',true,'Both collectors need to confirm the public meetup plan.'],['arrived','Confirm arrival',safe,'Waiting for both safety confirmations.'],['inspected','Inspect the other set',arrived,'Waiting for both arrival confirmations.'],['confirm','Confirm handoff and start swap',inspected,'Waiting for both inspections.']];return '<div class="bc-shared-checklist"><div class="bc-shared-checklist-head"><b>Shared live checklist</b><span class="bc-small">Your status is on the left; theirs updates automatically.</span></div>'+rows.map((r,i)=>flowStep(e,obj,r[0],r[1],r[2],r[3],i+1)).join('')+'</div>'}
function flowMarkup(e,meet,ret){const p=exchangePhase(e,meet,ret),k=p[0],returning=k==='return'||k==='return-plan';if(k==='paused')return '<span class="bc-pill red">Paused</span><h2>'+p[1]+'</h2><p class="bc-muted">'+p[2]+'</p><div class="bc-notice bad">Keep the conversation here. Contact <a href="mailto:'+SUPPORT_EMAIL+'">'+SUPPORT_EMAIL+'</a> if you need help.</div>';if(k==='complete')return '<span class="bc-pill green">Complete</span><h2>Exchange complete ✓</h2><p class="bc-muted">'+p[2]+'</p><div class="bc-notice good">Both sets were returned, inspected and confirmed.</div>';if(k==='plan'||k==='return-plan')return '<span class="bc-pill gold">Your shared next step</span><h2>'+p[1]+'</h2><p class="bc-muted">'+p[2]+'</p><div class="bc-next-action"><div><b>Either collector can set the plan</b><small>It appears immediately in this workspace for both of you.</small></div><button class="bc-btn primary" data-schedule="'+(returning?'return':'outbound')+'">'+(returning?'Schedule return meetup':'Schedule first meetup')+'</button></div>';const plan=returning?ret:meet;return '<span class="bc-pill gold">Shared live progress</span><h2>'+p[1]+'</h2><p class="bc-muted">'+p[2]+'</p><div class="bc-meetup-summary"><div><b>📍 '+esc(plan.venue_name||'Public venue')+'</b><br><span>'+fmtDateTime(plan.meetup_at)+'</span></div><button class="bc-btn ghost" data-schedule="'+(returning?'return':'outbound')+'">Update plan</button></div>'+sharedChecklist(e,plan,returning)+'<div class="bc-flow-safety"><button class="bc-btn danger" data-report="'+(returning?'return':'outbound')+'">Report an issue</button></div>'}
function requestCard(r){const incoming=r.responder_id===S.user.id,other=incoming?r.requester_id:r.responder_id,p=S.profiles[other]||{},offer=itemName(r.offered_item_id),want=itemName(r.requested_item_id);return '<article class="bc-card bc-pad bc-request bc-proposal-card"><div>'+pill(incoming?'Your decision':'Waiting for their decision',incoming?'gold':'blue')+'<h3>'+esc(offer)+' ⇄ '+esc(want)+'</h3><div class="bc-ex-meta">With '+esc(p.display_name||'Collector')+' · '+r.duration_days+' days</div>'+(r.message?'<div class="bc-notice" style="margin-top:10px"><b>Message</b><br>'+esc(r.message)+'</div>':'')+'<div class="bc-notice good" style="margin-top:10px"><b>After acceptance</b><br>Both sets are reserved and one shared workspace opens for meetup, handoff, return and messages.</div></div><div class="bc-request-actions">'+(incoming?'<button class="bc-btn primary" data-request-action="accept" data-request-id="'+r.id+'">Accept & reserve sets</button><button class="bc-btn" data-request-action="decline" data-request-id="'+r.id+'">Decline</button>':'<button class="bc-btn danger" data-request-action="cancel" data-request-id="'+r.id+'">Cancel proposal</button>')+'</div></article>'}

...
const LIFECYCLE_NOTIFICATION_KINDS=new Set(['exchange_accepted','exchange_declined','exchange_cancelled','exchange_created','meetup_proposed','swap_started','return_meetup_proposed','return_overdue','return_dispute','return_completed','message_received','reciprocal_match']);
...
const S={
  ...
  exchangeCapabilities:{checked:false,releaseItem:false,contractVersion:0},
...
let exchangeChannelId='';
let exchangeRefreshTimer=null;
let exchangeRefreshRunning=false;
let exchangeRefreshPending=false;
...
function clearProtectedState(){S.user=null;S.profile=null;S.collection=[];S.wishlist=[];S.matches=[];S.requests=[];S.exchanges=[];S.notifications=[];S.messages=[];S.reviews=[];S.liquidity=null;S.membership=null;S.exchangeCapabilities={checked:false,releaseItem:false,contractVersion:0};S.profiles={};S.items={};S.sets={}}
...
async function refreshCore(){
  ...
  const uid=S.user.id;
  const [p,c,w,req,ex,n,msg,liq,membership,exchangeCapabilities]=await Promise.all([
    ...
    settledTimeout(db.rpc('bc_exchange_capabilities'),5000)
  ]);
  ...
  const capabilityData=Array.isArray(exchangeCapabilities.data)?exchangeCapabilities.data[0]:exchangeCapabilities.data;
  S.exchangeCapabilities={checked:true,releaseItem:!exchangeCapabilities.error&&capabilityData?.release_item===true&&Number(capabilityData?.contract_version)>=1,contractVersion:Number(capabilityData?.contract_version||0)};
  if(exchangeCapabilities.error){const code=String(exchangeCapabilities.error.code||'unavailable').slice(0,40);console.warn('BrickCircle exchange release capability is unavailable.',{code});track('exchange_release_capability_unavailable',{code})}
...
async function openNotification(notification){
  ...
  const exchangeId=notification?.metadata?.exchange_id;
  if(exchangeId){navigate('exchange',exchangeId);return}
  if(notification?.kind==='message_received'){showInbox();return}
  shell();await renderRoute();
}
function stopExchangeRealtime(){
  clearTimeout(exchangeRefreshTimer);exchangeRefreshTimer=null;exchangeRefreshPending=false;exchangeChannelId='';
  const channel=exchangeChannel;exchangeChannel=null;if(!channel)return;
  try{const removal=db.removeChannel?.(channel);removal?.catch?.(()=>channel.unsubscribe?.())}catch(_){try{channel.unsubscribe?.()}catch(__){}}
}
async function reconcileOpenExchange(exchangeId){
  if(exchangeRefreshRunning){exchangeRefreshPending=true;return}
  exchangeRefreshRunning=true;
  try{
    do{
      exchangeRefreshPending=false;
      if(!S.user||routeName()!=='exchange'||routeId()!==exchangeId||exchangeChannelId!==exchangeId)break;
      await refreshCore();
      if(!S.user||routeName()!=='exchange'||routeId()!==exchangeId||exchangeChannelId!==exchangeId)break;
      await renderRoute();
    }while(exchangeRefreshPending)
  }finally{exchangeRefreshRunning=false}
}
function queueOpenExchangeRefresh(exchangeId){
  if(routeName()!=='exchange'||routeId()!==exchangeId||exchangeChannelId!==exchangeId)return;
  clearTimeout(exchangeRefreshTimer);exchangeRefreshTimer=setTimeout(()=>reconcileOpenExchange(exchangeId),200);
}
function startExchangeRealtime(exchangeId){
  const id=String(exchangeId||'');if(!S.user?.id||!id||typeof db.channel!=='function')return stopExchangeRealtime();
  if(exchangeChannel&&exchangeChannelId===id)return;
  stopExchangeRealtime();exchangeChannelId=id;
  const refresh=()=>queueOpenExchangeRefresh(id);
  exchangeChannel=db.channel(`exchange-workspace:${S.user.id}:${id}`)
    .on('postgres_changes',{event:'*',schema:'public',table:'exchanges',filter:`id=eq.${id}`},refresh)
    .on('postgres_changes',{event:'*',schema:'public',table:'exchange_meetups',filter:`exchange_id=eq.${id}`},refresh)
    .on('postgres_changes',{event:'*',schema:'public',table:'exchange_returns',filter:`exchange_id=eq.${id}`},refresh)
    .on('postgres_changes',{event:'INSERT',schema:'public',table:'messages',filter:`exchange_id=eq.${id}`},refresh)
    .subscribe(status=>{if(status==='CHANNEL_ERROR'||status==='TIMED_OUT')track('exchange_workspace_realtime_unavailable',{status})});
}
...
function activeExchanges(){return S.exchanges.filter(e=>!['completed','cancelled','released'].includes(e.state))}
...
function exchangeHistory(){return S.exchanges.filter(e=>['completed','cancelled','released'].includes(e.state))}
function workflowForItem(itemId){return activeExchanges().find(e=>e.item_a===itemId||e.item_b===itemId)||pendingRequests().find(r=>r.offered_item_id===itemId||r.requested_item_id===itemId)||null}
...
async function renderRoute(){
  ...
  if(r!=='exchange')stopExchangeRealtime();
...
function renderSetsBody(){
  const host=$('#bc-sets-body');if(!host)return;if(S.setTab==='wishlist'){host.innerHTML=S.wishlist.length?`<div class="bc-set-list">${S.wishlist.map(w=>mySetRow(w,true)).join('')}</div>`:empty('♡','Build Sets I Want','Add at least three sets you would genuinely like to experience. This gives BrickCircle enough demand signals to find reciprocal collectors.','Browse LEGO sets','browse')}else{host.innerHTML=S.collection.length?`<div class="bc-notice warn" style="margin-bottom:12px"><b>Make at least one set available to exchange.</b> Only sets you explicitly make available can appear in reciprocal matches.</div><div class="bc-set-list">${S.collection.map(c=>mySetRow(c,false)).join('')}</div>`:empty('🧱','Start with the LEGO you already own','Add at least three collectible sets. You decide which physical copies are available for a temporary local exchange.','Add my first set','browse')};bindCommon(host);$$('[data-exchangeable]',host).forEach(x=>x.onchange=()=>setExchangeable(x.dataset.exchangeable,x.checked));$$('[data-remove-wish]',host).forEach(b=>b.onclick=()=>removeWish(b.dataset.removeWish));$$('[data-change-photo]',host).forEach(b=>b.onclick=()=>{const item=S.collection.find(row=>row.id===b.dataset.changePhoto);if(item)replaceOwnerPhoto(item)});$$('[data-release-set]',host).forEach(b=>b.onclick=()=>releaseSetFromWorkflow(b.dataset.releaseSet));$$('[data-edit-set]',host).forEach(b=>b.onclick=()=>editCollectionItem(b.dataset.editSet));wireImages(host);if(S.setTab==='collection')hydrateOwnerPhotos(host)
}
function mySetRow(row,wish){const s=row.lego_sets||S.sets[row.set_number]||{},image=imageSetNumber(row.set_number),workflow=!wish&&workflowForItem(row.id),canRelease=S.exchangeCapabilities.releaseItem;return `<article class="bc-card bc-myset"> <div class="bc-myset-visual"><img src="https://images.brickset.com/sets/images/${attr(image)}.jpg" alt="" loading="lazy" data-set-image="${attr(row.set_number)}"><div hidden>🧱</div></div><div><h3>${esc(s.name||row.set_number)}</h3><div class="bc-myset-meta">Set ${esc(row.set_number)}${s.theme?` · ${esc(s.theme)}`:''}${s.piece_count?` · ${Number(s.piece_count).toLocaleString()} pieces`:''}</div><div class="bc-statusrow">${wish?pill('Sets I Want','blue'):(row.available_for_exchange?pill('Available to Exchange','green'):pill(workflow?'Reserved for Exchange':'Not Available'))}${!wish&&row.condition?pill(row.condition):''}${!wish&&row.completeness!=null?pill(`${row.completeness}% complete`):''}</div></div><div class="bc-myset-actions">${wish?`<button class="bc-btn" data-remove-wish="${attr(row.set_number)}" aria-label="Remove ${attr(s.name||row.set_number)} from Sets I Want">Remove</button>`:`<label class="bc-toggle"><input type="checkbox" data-exchangeable="${attr(row.id)}" ${row.available_for_exchange?'checked':''} ${workflow?'disabled':''}> Available to Exchange</label>${workflow&&canRelease?`<button class="bc-btn danger" data-release-set="${attr(row.id)}">Release this set from this exchange</button>`:workflow?'<span class="bc-small" data-release-unavailable>Exchange release is temporarily unavailable. Your reservation has not changed.</span>':''}<button class="bc-btn" data-change-photo="${attr(row.id)}">Change photo</button><button class="bc-btn" data-edit-set="${attr(row.id)}">Details</button>`}</div></article>`}
async function releaseSetFromWorkflow(itemId){
  const item=S.collection.find(row=>row.id===itemId);if(!item||!S.user)return;
  if(!S.exchangeCapabilities.releaseItem){track('exchange_release_blocked_capability',{contract_version:S.exchangeCapabilities.contractVersion});return toast('Exchange release is temporarily unavailable. Your reservation has not changed.')}
  const workflow=workflowForItem(itemId),active=workflow&&'state' in workflow&&workflow.state==='swap_active';
  const warning=active?'This set has already been handed over. BrickCircle will open the exchange so you can request an early return, contact the participant, or report an issue.':'This will end the pending exchange for both collectors, make both sets available again, notify the other participant, and preserve the audit history.';
  if(!confirm(`Release ${item.lego_sets?.name||item.set_number} from this exchange?\n\n${warning}`))return;
  const {data,error}=await db.rpc('release_exchange_item',{p_item_id:itemId,p_reason:null});
  if(error)return fail(error,'Could not release this set. Please try again.');
  if(data?.requires_early_return){toast(data.message||'Use the return or issue workflow after handoff.');return navigate('exchange',data.exchange_id)}
  await refreshCore();renderSetsBody();toast('This set has been released and is available for a new match.');
}
...
async function removeCollectionItem(id,name,button){
  ...
  try{
    ...
    const active=(refs||[]).find(row=>!['completed','cancelled','released'].includes(row.state));
...
function flowMarkup(e,meet,ret){
  ...
  if(e.state==='released')return `<h2>Set released</h2><div class="bc-notice good">This exchange ended before handoff. Both LEGO sets are available for a new match, and the history is preserved here.</div>`;
  if(e.state==='cancelled')return `<h2>Exchange cancelled</h2><div class="bc-notice">This exchange ended before handoff. Its history is preserved here.</div>`;
...
function handleAuthChange(event,session){
  ...
  if(event==='SIGNED_OUT'||event==='CONFIRMED_SIGNED_OUT'){stopExchangeRealtime();stopNotificationRealtime();clearNotificationPresentation();clearProtectedState()}
...
window.addEventListener('pagehide',()=>{lastHiddenAt=Date.now();rememberRoute();stopExchangeRealtime()});

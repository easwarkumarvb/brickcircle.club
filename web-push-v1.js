...
function injectStyle(){
  ...
  style.textContent='.bc-push-prompt{position:fixed;inset:0;z-index:10020;background:rgba(17,24,39,.54);display:grid;place-items:center;padding:20px}.bc-push-prompt.automatic{inset:76px 16px auto auto;background:transparent;padding:0;display:block;pointer-events:none}.bc-push-card{width:min(100%,460px);background:#fff;border:1px solid #e5e7eb;border-radius:18px;box-shadow:0 24px 70px rgba(17,24,39,.24);padding:22px}.bc-push-prompt.automatic .bc-push-card{width:min(calc(100vw - 32px),420px);pointer-events:auto}.bc-push-card h2{margin:0 0 8px;font:750 21px/1.25 system-ui;color:#111827}.bc-push-card p{margin:0;color:#4b5563;font:15px/1.5 system-ui}.bc-push-actions{display:flex;justify-content:flex-end;gap:9px;margin-top:18px;flex-wrap:wrap}.bc-push-actions button{border-radius:10px;padding:10px 14px;font:650 14px/1 system-ui;cursor:pointer}.bc-push-later{border:1px solid #d1d5db;background:#fff;color:#111827}.bc-push-enable{border:1px solid #111827;background:#111827;color:#fff}.bc-push-enable:disabled{opacity:.6;cursor:wait}@media(max-width:520px){.bc-push-prompt:not(.automatic){align-items:end;padding:12px}.bc-push-prompt.automatic{inset:68px 12px auto 12px}.bc-push-card{border-radius:18px 18px 14px 14px;padding:20px}.bc-push-actions button{min-height:44px;flex:1}}';
...
function open(user,{automatic=false}={}){
  ...
  close();injectStyle();
  const root=document.createElement('div');root.className=`bc-push-prompt${automatic?' automatic':''}`;root.setAttribute('role',automatic?'region':'dialog');if(!automatic)root.setAttribute('aria-modal','true');root.setAttribute('aria-labelledby','bc-push-title');

...
test('join entry reliability layer loads immediately after the app',()=>{
  ...
  const auth=html.indexOf('/v3-auth-onboarding-hotfix.js?v=20260829-2');
...
test('PWA cache includes the join reliability asset',()=>{
  const sw=read('catalogue-cache-sw.js');
  expect(sw).toContain("brickcircle-shell-v3-20260829-joinfix1");

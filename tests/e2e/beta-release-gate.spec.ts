...
test('Join free opens the auth UX promptly on mobile', async ({page})=>{
  ...
  await expect(page.locator('#bc-overlay')).toBeVisible({timeout:4000});
  ...
  for(let batch=0;batch<10;batch++){
    await Promise.all(Array.from({length:10},async(_,j)=>{
      ...
      try{
        ...
        await page.locator('a[href="/v2.html?join=1"]').first().click({timeout:5000});
        await page.locator('#bc-overlay').waitFor({state:'visible',timeout:5000});
        const text=await page.locator('#bc-overlay').innerText();
        if(!/Join BrickCircle/i.test(text))failures.push(`#${n}: wrong join content`);
        if(errors.length)failures.push(`#${n}: ${errors.join(' | ')}`);
...
test('Google sign-in helper parses OAuth URL without constructor errors', async ({page})=>{
  const errors=collectErrors(page);
  ...
  await page.evaluate(()=>{
    ...
    (window as any).supabase={createClient:()=>({
      auth:{
        ...
        signInWithIdToken:async()=>{(window as any).__idTokenCalled=true;return {error:null}}
      },
...
test('signed-in collector can add a set, wishlist, mark exchangeable, match and propose through UX', async ({page})=>{
  const errors=collectErrors(page);
  await page.setViewportSize({width:390,height:844});
  ...
  await page.evaluate(()=>{
    ...
    const chain=(table:string)=>{
      const q:any={table,filters:[],patch:null,mode:'select',payload:null,
        ...
        then(resolve:any){let data:any=[];if(table==='profiles')data=[profile];else if(table==='collection_items')data=state.collection.map((x:any)=>({...x,lego_sets:sets.find((s:any)=>s.set_number===x.set_number)}));else if(table==='wishlists')data=state.wishlist.map((x:any)=>({...x,lego_sets:sets.find((s:any)=>s.set_number===x.set_number)}));else if(table==='exchange_requests')data=state.requests;else if(table==='lego_sets')data=sets;else if(table==='public_profiles')data=[{id:'00000000-0000-4000-8000-000000000099',display_name:'Match Collector',country:'India',city:'Bengaluru',rating:5,review_count:3,identity_verified:false,member_since:new Date().toISOString(),founding_member_number:9}];else data=[];
          ...
          if(q.mode==='delete'&&table==='wishlists'){state.wishlist=state.wishlist.filter((x:any)=>!q.filters.every(([k,v]:any[])=>x[k]===v));data=null}
    ...
    const db={
      ...
      rpc:async(name:string,args:any)=>{
        ...
        if(name==='find_matches'){if(!state.collection.some((x:any)=>x.available_for_exchange)||!state.wishlist.length)return {data:[],error:null};const own=state.collection.find((x:any)=>x.available_for_exchange),wish=state.wishlist[0],os=sets.find((s:any)=>s.set_number===own.set_number),ws=sets.find((s:any)=>s.set_number===wish.set_number);return {data:[{match_user:'00000000-0000-4000-8000-000000000099',match_score:95,offered_name:os.name,offered_set:os.set_number,offered_value:450,offered_item:own.id,requested_name:ws.name,requested_set:ws.set_number,requested_value:450,requested_item:'other-item-1'}],error:null}}
  ...
  await expect(page.locator('.bc-welcome')).toContainText(/QA Collector/i,{timeout:5000});

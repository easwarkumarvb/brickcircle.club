...
from (
  select r.requester_id as recipient_id,r.id as request_id
  from public.exchange_requests r
  where r.status='pending' and exists (
    select 1 from public.exchanges e
    where e.state not in ('completed','cancelled')
      and (e.item_a in (r.offered_item_id,r.requested_item_id) or e.item_b in (r.offered_item_id,r.requested_item_id))
  )
  union all
  select r.responder_id as recipient_id,r.id as request_id
  from public.exchange_requests r
  where r.status='pending' and exists (
    select 1 from public.exchanges e
    where e.state not in ('completed','cancelled')
...
revoke all on function public.find_matches(uuid) from anon;
grant execute on function public.find_matches(uuid) to authenticated;
revoke all on function public.queue_new_reciprocal_match_notifications(uuid) from public;
revoke all on function public.queue_new_reciprocal_match_notifications(uuid) from anon;
revoke all on function public.queue_new_reciprocal_match_notifications(uuid) from authenticated;

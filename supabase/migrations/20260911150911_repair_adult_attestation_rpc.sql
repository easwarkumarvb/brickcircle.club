-- Repair the deployed 18+ attestation RPC contract.
-- This intentionally repeats the self-contained contract from the original
-- attestation rollout so it is safe when that rollout was missed in an
-- environment, while preserving every already-recorded confirmation.
...
-- Force PostgREST to expose the exact newly-restored RPC signature immediately.
